...
const AvailabilityChecker = dynamic(
    () =>
        import("@/components/availability-checker").then(
            (mod) => mod.AvailabilityChecker
        ),
    { ssr: true }
)
...
const WhatsAppCTA = dynamic(
    () =>
        import("@/components/whatsapp-cta").then(
            (mod) => mod.WhatsAppCTA
        ),
    { ssr: true }
)
...
const FaqsSection = dynamic(
    () =>
        import("@/components/faqs-section").then(
            (mod) => mod.FaqsSection
        ),
    { ssr: true }
)

const VenueRequirements = dynamic(
    () =>
        import("@/components/venue-requirements").then(
            (mod) => mod.VenueRequirements
        ),
    { ssr: true }
)
...
const FloatingNavigation = dynamic(
    () =>
        import("@/components/floating-navigation").then(
...
const MultiVideoReels = dynamic(
    () =>
        import("@/components/multi-video-reels").then(
            (mod) => mod.MultiVideoReels
        ),
    {
        ssr: false,
        loading: () => <div className="h-64" />,
...
export default function MagazinePhotoBoothPage() {
    ...
    const navigationSections = [
        ...
        { id: "faq-section", label: "FAQs", icon: MessageCircle },
    ]
